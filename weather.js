...
// if you would like enterprise support, please contact me [duncanjdiii@gmail.com]
...
function computeImage(forecast) {
	...
	if (fc.includes("chance")) {
		image += "maybe";
	}
	if (
		fc.includes("partly") ||
		fc.includes("decreasing") ||
		fc.includes("a few")
	) {
		image += "partly";
	}
	if (fc.includes("heavy") || fc.includes("severe")) {
		image += "heavy";
	}
	if (fc.includes("wintry mix")) {
		image += "sleety";
	}
	if (fc.includes("storm")) {
		...
		fc.includes("rain") ||
		(fc.includes("showers") && !fc.includes("snow"))
	) {
	...
	} else if (fc.includes("drizzle") || fc.includes("sprinkles")) {
		image += "drizzly";
	}
	if (fc.includes("snow") || fc.includes("flurries")) {
		image += "snowy";
	}
	if (fc.includes("frost")) {
		image += "frosty";
	}
	if (fc.includes("fog") || fc.includes("mist")) {
		image += "foggy";
	}
	if (fc.includes("sunny")) {
		image += "sunny";
	}
	if (fc.includes("clear") || fc.includes("fair")) {
		image += "sunny";
	}
	if (fc.includes("hot")) {
		image += "hot";
	}
	if (fc.includes("cloud") || fc.includes("overcast")) {
		image += "cloudy";
	}
	//if(fc.includes("breez") || fc.includes("wind") || fc.includes("blust")) { image= "windy"; }
	if (fc.includes("smoke")) {
		image = "smokey";
	}
	if (fc.includes("obscure")) {
		image = "obscured";
	}
	if (fc == "sunny") {
		image = "sunny";
	}
	if (fc == "" || fc == "na" || fc.includes("unknown")) {
		image = true;
	}
	return image;
}
			...
			x.onreadystatechange = () => {
				if (x.readyState == 4 && x.status == 200) {
					...
					if (tLat != null && tLong != null) {
						...
						var bigSwitchImg = document.createElement("img");
						bigSwitchImg.setAttribute("src", "css/img/bigswitch.xbm.gif");
						bigSwitchImg.setAttribute("title", "grayscale");
						bigSwitchImg.setAttribute("style", "cursor: pointer;");
						weatherBox.appendChild(bigSwitchImg);

						bigSwitchImg.addEventListener("click", () => {
							const isExpanding = bigSwitchImg.alt === "color";

							if (isExpanding) {
								bigSwitchImg.alt = "grayscale";
								bigSwitchImg.title = "grayscale";
								bigSwitchImg.src = "css/img/bigswitch.xbm.gif";

								weatherBox.querySelectorAll("img").forEach((img) => {
									const src = img.src;
									const newSrc = src.replace(/_bw(\.[^.]+)$/, "$1");
									img.src = newSrc;
								});

								document
									.querySelectorAll("[class*='bar-']")
									.forEach((element) => {
										element.classList.forEach((className) => {
											if (
												...
												element.classList.remove(className + "-bw");
											}
										});
									});

								const mainElement = document.getElementsByTagName("main")[0];
								mainElement.classList.remove("bw");
							} else {
								bigSwitchImg.alt = "color";
								bigSwitchImg.title = "color";
								bigSwitchImg.src = "css/img/bigswitch_on.xbm.gif";

								weatherBox.querySelectorAll("img").forEach((img) => {
									const src = img.src;
									if (
										src.includes("face") &&
										!["today", "tonight"].some((keyword) =>
										...
										if (lastDotIndex !== -1) {
											const newSrc =
												src.slice(0, lastDotIndex) +
												"_bw" +
												src.slice(lastDotIndex);
											img.src = newSrc;
										}
									}
								});
									...
									.forEach((element) => {
										element.classList.forEach((className) => {
											if (
												...
												element.classList.add(className, className + "-bw");
											}
										});
									});

								const mainElement = document.getElementsByTagName("main")[0];
								mainElement.classList.add("bw");
							}

							bigSwitchImg.prevSrc = bigSwitchImg.src;
						});

						bigSwitchImg.addEventListener("mousedown", (event) => {
							bigSwitchImg.src = bigSwitchImg.src.replace(
								/(\.xbm\.gif)$/,
								"_pressed$1",
							);
					...
					for (var i = 0; i < centerName.length; i++) {
						if (
							(tmpCounter >= 9 && centerName.charAt(i) == " ") ||
							centerName.charAt(i) == ","
						) {
							...
							if (
								...
								toAdd.setAttribute("src", "weather/MISC/question/face.gif");
							} else {
								...
								if (bw) {
									toAdd.setAttribute(
										"src",
										"weather/letters/" + toReplace + "_bw.gif",
									);
								} else {
									toAdd.setAttribute(
										"src",
										"weather/letters/" + toReplace + ".gif",
									);
								}
							}
					...
					toAdd.setAttribute("src", "weather/day/today/face.gif");
					weatherBox.appendChild(toAdd);
					weatherBox.appendChild(br);
					...
					if (data.time.startPeriodName[0] == "Overnight") {
						...
						startTime = startTimeT.substring(
							startTimeT.indexOf("T") + 1,
							startTimeT.indexOf(":"),
						);
						if (startTime != 23) {
							count = 2;
					...
					// special circumstances
					if (
						data.time.startPeriodName[0] == "Tonight" ||
						...
						toAdd.setAttribute("src", "weather/day/tonight/face.gif");
						weatherBox.appendChild(toAdd);
						var br = document.createElement("br");
						weatherBox.appendChild(br);
						tonight = true;
					...
					for (j; j < tempArray.length; j++) {
						if (
							(j == 1 && tonight) ||
							(j == 2 && tonight && (afternoon || today))
						) {
							var toAdd = document.createElement("hr");
							weatherBox.appendChild(toAdd);

							counter++;
							if (counter == weekday.length) {
								counter = 0;
							}
							n = weekday[counter];
						}
					...
					function lvr() {
						...
						if (lv.src.indexOf("up") !== -1) {
							fav.href =
								fav
									.getAttribute("href")
									.substring(0, fav.getAttribute("href").lastIndexOf(".")) +
								"_bw.gif";
							for (var i = 0; i < piconArray.length; i++) {
								piconArray[i].src =
									piconArray[i].src.substring(
										0,
										piconArray[i].src.lastIndexOf("."),
									) + "_bw.gif";
							}
							for (var i = 0; i < dayArray.length; i++) {
								dayArray[i].src =
									dayArray[i].src.substring(
										0,
										dayArray[i].src.lastIndexOf("."),
									) + "_bw.gif";
							}
							for (var i = 0; i < thermArray.length; i++) {
								thermArray[i].src =
									thermArray[i].src.substring(
										0,
										thermArray[i].src.lastIndexOf("."),
									) + "_bw.gif";
							}
							for (var i = 0; i < letterArray.length; i++) {
								letterArray[i].src =
									letterArray[i].src.substring(
										0,
										letterArray[i].src.lastIndexOf("."),
									) + "_bw.gif";
							}
							for (var i = 0; i < windArray.length; i++) {
								windArray[i].src =
									windArray[i].src.substring(
										0,
										windArray[i].src.lastIndexOf("."),
									) + "_bw.gif";
							}
							...
							fav.href =
								fav
									...
									.substring(
										0,
										fav.getAttribute("href").lastIndexOf("_bw.gif"),
									) + ".gif";
							for (var i = 0; i < piconArray.length; i++) {
								piconArray[i].src =
									piconArray[i].src.substring(
										0,
										piconArray[i].src.lastIndexOf("_bw.gif"),
									) + ".gif";
							}
							for (var i = 0; i < dayArray.length; i++) {
								dayArray[i].src =
									dayArray[i].src.substring(
										0,
										dayArray[i].src.lastIndexOf("_bw.gif"),
									) + ".gif";
							}
							for (var i = 0; i < thermArray.length; i++) {
								thermArray[i].src =
									thermArray[i].src.substring(
										0,
										thermArray[i].src.lastIndexOf("_bw.gif"),
									) + ".gif";
							}
							for (var i = 0; i < letterArray.length; i++) {
								letterArray[i].src =
									letterArray[i].src.substring(
										0,
										letterArray[i].src.lastIndexOf("_bw.gif"),
									) + ".gif";
							}
							for (var i = 0; i < windArray.length; i++) {
								windArray[i].src =
									windArray[i].src.substring(
										0,
										windArray[i].src.lastIndexOf("_bw.gif"),
									) + ".gif";
							}
							for (var i = 0; i < barPrecipArray.length; i++) {
					...
					var meta = [];
					var currentForecast =
						data.currentobservation.Weather.toString().toLowerCase();

					var link = document.createElement("link");
					link.type = "text/css";
					link.rel = "stylesheet";
					link.href = "worlds/istyle.css";
					meta.push(link);

					var scr = document.createElement("script");
					scr.src = "worlds/oop.js";
					meta.push(scr);

					var scr = document.createElement("script");
					scr.src = "worlds/tools.js";
					meta.push(scr);

					var scr = document.createElement("script");
					scr.src = "worlds/cookie.js";
					meta.push(scr);

					var scr = document.createElement("script");
					scr.src = "worlds/palette.js";
					meta.push(scr);

					var scr = document.createElement("script");
					scr.src = "worlds/bitmap.js";
					meta.push(scr);

					var scr = document.createElement("script");
					scr.src = "worlds/scenes.js";
					meta.push(scr);

					var scr = document.createElement("script");
					scr.src = "worlds/tween.js";
					meta.push(scr);

					var scr = document.createElement("script");
					scr.src = "worlds/framecount.js";
					meta.push(scr);

					var scr = document.createElement("script");
					scr.src = "worlds/imain.js";
					meta.push(scr);

					/*var scr = document.createElement("script");
				  scr.src = "worlds/initialize.js";
				  meta.push(scr);*/

					var hasAddedLandscapes = false;
					if (container && window.localStorage) {
						landscapeCheckbox.checked =
							localStorage.getItem("landscapesEnabled") == "true";

						if (localStorage.getItem("landscapesEnabled") == "true") {
							for (i = 0; i < meta.length; i++) {
								document.body.appendChild(meta[i]);
								// document.body.insertBefore(meta[i], document.body.lastChild);
							}
...
function helper(curTemp, precip, forecast, showDate, text, winds, windd) {
	...
	if (bw) {
		toAdd.setAttribute("src", "weather/day/" + n + "/face_bw.gif");
	} else {
		toAdd.setAttribute("src", "weather/day/" + n + "/face.gif");
	}
	var d = new Date();
	...
	if (!showDate) {
		...
		if (text.toLowerCase().includes("cloudy")) {
			if (bw) {
				toAdd.setAttribute("src", "weather/sky/partlymoonny/face_bw.gif");
			} else {
				toAdd.setAttribute("src", "weather/sky/partlymoonny/face.gif");
			}
		} else {
			if (bw) {
				toAdd.setAttribute("src", "weather/sky/moonny/face_bw.gif");
			} else {
				toAdd.setAttribute("src", "weather/sky/moonny/face.gif");
			}
		}
		/*hasShownMoon = true;
		  } else {
	...
	var reg =
		"(?:calm wind becoming )*(south(?:west|east)?|north(?:west|east)?|east|west)+? (?:wind|around)(?:.* mph)*";
	const matches = text.toLowerCase().match(reg);
	if (matches) {
		var toAdd = document.createElement("img");
		...
		if (bw) {
			toAdd.setAttribute("src", "weather/wind/" + matches[1] + "/face_bw.gif");
		} else {
			toAdd.setAttribute("src", "weather/wind/" + matches[1] + "/face.gif");
		}
		...
		if (bw) {
			if (windd >= 337.5 || windd < 22.5) {
				toAdd.setAttribute("src", "weather/wind/north/face_bw.gif");
			} else if (windd >= 22.5 && windd < 67.5) {
				toAdd.setAttribute("src", "weather/wind/northeast/face_bw.gif");
			} else if (windd >= 67.5 && windd < 112.5) {
				toAdd.setAttribute("src", "weather/wind/east/face_bw.gif");
			} else if (windd >= 112.5 && windd < 157.5) {
				toAdd.setAttribute("src", "weather/wind/southeast/face_bw.gif");
			} else if (windd >= 157.5 && windd < 202.5) {
				toAdd.setAttribute("src", "weather/wind/south/face_bw.gif");
			} else if (windd >= 202.5 && windd < 247.5) {
				toAdd.setAttribute("src", "weather/wind/southwest/face_bw.gif");
			} else if (windd >= 247.5 && windd < 292.5) {
				toAdd.setAttribute("src", "weather/wind/west/face_bw.gif");
			} else if (windd >= 292.5 && windd < 337.5) {
				toAdd.setAttribute("src", "weather/wind/northwest/face_bw.gif");
			}
		} else {
			if (windd >= 337.5 || windd < 22.5) {
				toAdd.setAttribute("src", "weather/wind/north/face.gif");
			} else if (windd >= 22.5 && windd < 67.5) {
				toAdd.setAttribute("src", "weather/wind/northeast/face.gif");
			} else if (windd >= 67.5 && windd < 112.5) {
				toAdd.setAttribute("src", "weather/wind/east/face.gif");
			} else if (windd >= 112.5 && windd < 157.5) {
				toAdd.setAttribute("src", "weather/wind/southeast/face.gif");
			} else if (windd >= 157.5 && windd < 202.5) {
				toAdd.setAttribute("src", "weather/wind/south/face.gif");
			} else if (windd >= 202.5 && windd < 247.5) {
				toAdd.setAttribute("src", "weather/wind/southwest/face.gif");
			} else if (windd >= 247.5 && windd < 292.5) {
				toAdd.setAttribute("src", "weather/wind/west/face.gif");
			} else if (windd >= 292.5 && windd < 337.5) {
				toAdd.setAttribute("src", "weather/wind/northwest/face.gif");
			}
		}
		windArray.push(toAdd);
		dayBox.appendChild(toAdd);
	...
	for (var i = 0; i < curTemp.length; i++) {
		...
		if (curTemp == "M" || curTemp == "" || curTemp == null || curTemp == "NA") {
			...
			toAdd.setAttribute("src", "weather/nums/" + curTemp.charAt(i) + ".gif");
		}
	...
	toAdd.className = "image-weather-letter";
	toAdd.setAttribute("src", "weather/nums/degree.gif");
	eworldBox.appendChild(toAdd);

	var toAdd = document.createElement("img");
	toAdd.setAttribute("id", "tempDegree");
	toAdd.className = "image-weather-letter";
	toAdd.setAttribute("src", "weather/nums/f.gif");
	eworldBox.appendChild(toAdd);
	...
	if (curTemp != "M" && curTemp != "" && curTemp != null && curTemp != "NA") {
		weatherPath = curTemp;
		while (weatherPath % 5 != 0) {
			weatherPath--;
		} // decrement the value to a valid picon therm
	...
	if (curTemp == "M" || curTemp == "" || curTemp == null || curTemp == "NA") {
		if (bw) {
			toAdd.setAttribute("src", "weather/MISC/question/face_bw.gif");
		} else {
			toAdd.setAttribute("src", "weather/MISC/question/face.gif");
		}
	} else {
		if (bw) {
			toAdd.setAttribute(
				"src",
				"weather/temp2/" + weatherPath + "/face_bw.gif",
			);
		} else {
			toAdd.setAttribute("src", "weather/temp2/" + weatherPath + "/face.gif");
		}
	}
	if (firstRun) {
		var link = document.createElement("link");
		...
		link.rel = "icon";
		if (bw) {
			link.href = "weather/temp/" + weatherPath + "/face_bw.gif";
		} else {
			link.href = "weather/temp/" + weatherPath + "/face.gif";
		}
	...
	if (forecast.includes("then")) {
		...
		var secondForecast = forecast.substring(
			forecast.indexOf("then") + 5,
			forecast.length,
		);
		...
		toAdd.onclick = function () {
			this.setAttribute("title", firstForecast);
		};
		...
		if (pth == true) {
			...
			if (bw) {
				toAdd.setAttribute("src", "weather/sky/" + pth + "/face_bw.gif");
			} else {
				toAdd.setAttribute("src", "weather/sky/" + pth + "/face.gif");
			}
		}
		...
		if (precip != null) {
			...
			for (var i = 0; i < precip.length; i++) {
				...
				if (curTemp == "M" || curTemp == "" || precip == "NA") {
					...
					toAdd.setAttribute(
						"src",
						"weather/nums/" + precip.charAt(i) + ".gif",
					);
				}
				precipBox.appendChild(toAdd);
			}
			if (curTemp != "M" && curTemp != "" && precip != "NA") {
				...
				toAdd.className = "image-weather-letter";
				toAdd.setAttribute("src", "weather/nums/percent.gif");
				precipBox.appendChild(toAdd);
			}
		...
		toAdd.setAttribute("title", text);
		toAdd.onclick = function () {
			this.setAttribute("title", secondForecast);
		};
		toAdd.className = "image-weather-table";
		...
		toAdd.setAttribute("title", text);
		toAdd.onclick = function () {
			this.setAttribute("title", forecast);
		};
		toAdd.className = "image-weather-table";
		...
		if (precip != null) {
			...
			if (curTemp != "M" && curTemp != "" && precip != "NA") {
				...
				addPercent.setAttribute("src", "weather/nums/percent.gif");
	...
	if (
		(!showDate && tmrw) ||
		((dCount == LAST_DAY || dCount == LAST_DAY + 1) && d.getHours() != 0)
	) {
		...
		for (var i = position; i < weatherProb[0].childNodes.length; i++) {
			...
			var simpTime = curTime.substring(
				curTime.indexOf("T") + 1,
				curTime.indexOf(":"),
			);
			...
			bar.setAttribute(
				"style",
				"height:" + precipProb[0].childNodes[i].textContent / 2 + "px;",
			);
			...
			if (cur != undefined) {
				fCast =
					cur.getAttribute("coverage") + " " + cur.getAttribute("weather-type");
			}
			bar.setAttribute(
				"title",
				precipProb[0].childNodes[i].textContent + "% " + fCast,
			);
			...
			bar.setAttribute(
				"style",
				"height:" + tempProb[0].childNodes[i].textContent / 2 + "px;",
			);
			...
			if (windChillProb[0]) {
				// for obvious reasons, this is only enabled during the fall and winter seasons
				//alert("wind chill!");
				...
				subBar.setAttribute(
					"style",
					"height:" + windChillProb[0].childNodes[i].textContent / 2 + "px;",
				);
				...
				subBar.setAttribute(
					"title",
					"Wind Chill: " + windChillProb[0].childNodes[i].textContent + "F",
				);
			}
			...
			bar.setAttribute(
				"style",
				"height:" + humidityProb[0].childNodes[i].textContent / 2 + "px;",
			);
			...
			bar.setAttribute(
				"title",
				"Humidity: " + humidityProb[0].childNodes[i].textContent + "%",
			);
			...
			subBar.setAttribute(
				"style",
				"height:" + dewPointProb[0].childNodes[i].textContent / 2 + "px;",
			);
			...
			subBar.setAttribute(
				"title",
				"Dew Point: " + dewPointProb[0].childNodes[i].textContent + "F",
			);
			...
			bar.setAttribute(
				"style",
				"height:" + cloudProb[0].childNodes[i].textContent / 2 + "px;",
			);
			...
			bar.setAttribute(
				"title",
				"Cloud Cover: " + cloudProb[0].childNodes[i].textContent + "%",
			);
			...
			subBar.setAttribute(
				"style",
				"height:" + windGustProb[0].childNodes[i].textContent / 2 + "px;",
			);
			...
			subBar.setAttribute(
				"title",
				"Wind Gust: " + windGustProb[0].childNodes[i].textContent + "MPH",
			);
		...
		for (var i = startTime; i < Number(startTime) + tCount; i++) {
			...
			for (var j = 0; j < strN.length; j++) {
				...
				hour.src = "weather/nums_cure/" + strN[j] + ".gif";
